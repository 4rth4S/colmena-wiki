...
## Auto-spawn mode (flat-team orchestration)

With `--auto-spawn`, `colmena mission spawn` generates an **`ORCHESTRATE.md`** file in the mission directory. This file contains step-by-step instructions for the operator's Claude Code session: team setup, Agent() calls for every worker as teammates, and delegation commands. The operator's session acts as **team-lead** — no intermediate Mission Lead agent is needed.

The flat-team pattern ensures each agent independently calls `review_submit`, so the ELO trust calibration cycle works per-agent across the whole squad.

In Claude Code, say **"orquestá esta misión"** and Claude handles the rest.
