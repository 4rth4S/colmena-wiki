...
- **Sequential** — Agents work one after another. Each receives the previous agent's output.
- **Fan-out-merge** — Multiple agents work in parallel, then a coordinator merges results.
- **Hierarchical** — A coordinator delegates to workers. Used by plan-then-execute.
- **Adversarial** — Two agents with opposing goals. Red team vs blue team.
- **Peer** — Agents work independently and share findings as peers.
- **Iterative** — Work cycles through improvement until quality criteria are met.
- **Recursive** — Problems decomposed recursively, each level adding detail.
